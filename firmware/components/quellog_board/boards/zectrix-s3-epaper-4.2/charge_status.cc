...
void ChargeStatus::Tick(int64_t now_ms) {
    ...
    const bool detect_stable =
        (detect_high_start_ms_ >= 0) && ((now_ms - detect_high_start_ms_) >= kStableHighMs);
    const bool full_stable =
        (full_high_start_ms_ >= 0) && ((now_ms - full_high_start_ms_) >= kStableHighMs);
    const bool alt_seen =
        power_present &&
        (last_detect_seen_ms_ >= 0) &&
        (last_full_seen_ms_ >= 0) &&
...
uint32_t ChargeStatus::Pack(State state, bool power_present, bool charging, bool full, bool no_battery) {
    return static_cast<uint32_t>(state) |
           (static_cast<uint32_t>(power_present) << 8) |
           (static_cast<uint32_t>(charging) << 9) |
           (static_cast<uint32_t>(full) << 10) |
           (static_cast<uint32_t>(no_battery) << 11);
}

ChargeStatus::Snapshot ChargeStatus::Unpack(uint32_t value) {
    Snapshot snapshot = {};
    snapshot.state = static_cast<State>(value & 0xFF);
    snapshot.power_present = (value >> 8) & 0x1;
    snapshot.charging = (value >> 9) & 0x1;
    snapshot.full = (value >> 10) & 0x1;
    snapshot.no_battery = (value >> 11) & 0x1;
    return snapshot;
}
